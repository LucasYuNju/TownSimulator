...
public class ConstructExecuteBTN implements ActionNode{
	...
	public ExcuteResult execute(float deltaTime) {
		...
		if(cnt > 0)
		{
			mConstructInfo.proj.getBuilding().addCurBuildContributes(cnt * BUILD_CONTRIBUTE_SPPED);
			System.out.println("Build " + mConstructInfo.proj.getBuilding().getCurBuildContributes() + "/" + mConstructInfo.proj.getBuilding().getNeededBuildContributes());
		}
		
		if(mConstructInfo.proj.getBuilding().isBuildFinished())
		{
